...
/***************************  FUNCION QUE AGREGA LOS ELEMENTOS ACTIVOS A LA LISTA SOBRE EL MAPA CON LAS OPCIONES POR REENGLON *******************************/	
		...
		function agregaActivos(distrito, llave, i, tipo, are, divisionId, tipoUser, elemento){									
			if (distrito==""){
				$("#areaZone").append('<div><input type="checkbox" id="'+llave+'" value="'+llave+'" checked onchange="toggleCapa(this.value, \''+are+'\', \''+distrito+'\');"/><label>'+llave+'</label></div>');
			}else if (distrito!=""){
				$("#distZone").append('<div><input type="checkbox" id="'+distrito+'" value="'+distrito+'" checked onchange="toggleDistrict(this.value,\''+llave+'\',\''+i+'\',\''+are+'\');"/> <label>'+llave+'-'+distrito+'</label></div>');
			}
			if (tipoUser==1){ //// SI ES USUARIO COMERCIAL
				...
				$("#techMenu").append('<div class="activoEnlaLista"  id="comic_'+llave+'" onclick="dialogModel(\''+llave+'\', \''+are+'\', \''+distrito+'\', \'tec\');" /></div>');
...
//shufle capas
	function toggleCapa(llave, are, distrito){
		if ($("#"+llave).is(':checked')) {
			...
			dataCartografica[llave].capas[llave].setMap(map);
			autoLoad(llave, are, '');
			$("#techMenu").html('<i class="fa fa-filter"></i><div class="showtitle"><small>Filtrar tecnologías</small></div><div class="activoEnlaLista" id="comic_Distritos-LINDAVISTA" onclick="dialogModel(\''+llave+'\', \''+are+'\', \''+distrito+'\', \'tec\');"></div>');
	...
	function toggleDistrict(distrito, op, i, are){
		var arrDist = dataCartografica[op].apiResponse[i];
