...
function altura(){ // sólo para detectar la altura de la ventana
	w=window.innerWidth;
...
Highcharts.theme={ // tema para los gráficos
...
function connect(){ // conexión a los sockets
...
function sendMail(a,m,c){ // para enviar correos
	var k={pky:'b.4{d2xA',M:m,A:a,C:c};
	$.when(promesas.all(k)).done(function(x){
		console.log(x);
	});
}
function creaMapa(s,t){ // para crear los mapas
	function estilos(){
...
function limpiaPoligonos(){ // para limpiar los mapas
	map.data.forEach(function(feature){
		map.data.remove(feature);
	});
}
function limpiaMarcadores(){ // para quitar los marcadores
	for(var i=0; i<marcadores.length; i++){marcadores[i].setMap(null);}
	marcadores=[];ubicalosFirst=1;
}
function procesaPoints(geometry,callback,thisArg){ // inserta los marcadores
...
function setColores(){
	map.data.setStyle(function(feature){ // para ponerle un color al poligono
		return({
...
function centraMapa(){ // para centrar el mapa a el contenido pintado
	var bounds=new google.maps.LatLngBounds();
...
function pintaPoligonos(gj){ // mete los poligonos obtenidos
	limpiaPoligonos();
	map.data.addGeoJson(gj);
	setColores();
}
function creaDivsAreas(t,z,y){ // obtine y pinta las divisiones
...
function creaAreas(t,z,y){ // obtiene y pinta las areas
...
function liderLogin(){ // solo se ejecuta si se logueo un lider, muestra información exclusiva para el
	var ubicandoFiLid=[];
	limpiaPoligonos();
...
function muestraUsuarios(){ // funcion para devolver los usuarios
...
function creaMapaCaliente(){ // para crear el mapa de calor en maps
	var mapB,heatmap,a=[];
	initMap();
	function initMap(){
...
function muestraGraficoReal(H){ // para traer los graficos de tiempo real
	...
	$.when(promesas.all({pky:'oP{ñ_,m$"',P:lis,R:misRegiones})).done(function(x){
		x=jQuery.parseJSON(x);
		if(x.errorMessage!='')
			creanotificacion('Error','<b>'+x.errorMessage,'','','error');
		else
...
function addSeries(x){ // para meter las series a los gragicos de barras
...
function grafMetas(){ // para pintar llos graficos de metas, este no se esta usando aún
	var mi=0,ma=0;
...
function creaOtrosGraficos(x){ // crea los otros graficos
...
function muestraReportes(){
	...
	$.when(promesas.all({pky:'ñ*}{Lokj'}),fechasR.a()).done(function(x){
...
function muestraCalendario(){
	...
	var calendario=$('#calendar').fullCalendar({
		...
		eventClick:function(calEvent,jsEvent,view){
			if(Rol=='Lider Promotor'){
				...
				$.when(promesas.all({pky:'.tr/(ydF',P:calEvent.idActividad})).done(function(x){
					x=jQuery.parseJSON(x);
					if(x.errorMessage!=''){
						$("#loading").hide();
						creanotificacion('Error',x.errorMessage,'','','error');
	...
	function GetCampasMias(){
		$.when(promesas.all({pky:'lj.m,-/5tD',P:misRegiones})).done(function(x){
			x=jQuery.parseJSON(x);
			if(x.errorMessage!='') creanotificacion('Error:',x.errorMessage,'','','error');
	...
	if(Rol=='Lider Promotor')
		...
		$.when(promesas.all({pky:'ñ*}{Lokj'})).done(function(x){
			x=jQuery.parseJSON(x);
			if(x.hasOwnProperty("errorMessage"))
				creanotificacion('Error','<b>'+x.errorMessage,'','','error');
			else{
...
function meteRegionaCR(asig){
	...
	if(!!~jQuery.inArray(region,RegionesCR)){
		...
		$.when(promesas.all({pky:'}-.Ygf#44',P:id_edit,R:region})).done(function(x){
			x=JSON.parse(x);
			if(x.errorMessage!=''){
				console.log(x);
				creanotificacion('Error',x.errorMessage,'','','error');
				...
				asig.append("<a data-id='"+x.Resp[0].id+"' data='"+region.original+"'>"+region.regionT+"</a>");
			}
		});
	}
...
function nuevoUsuario(){
	...
	$.when(dialogos(forma,size)).done(function(x){
		...
		pass.add(Validate.Format, {pattern: /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[$@$!%*?&])[A-Za-z\d$@$!%*?&]/});
		pass.add(Validate.Length,{minimum: 6, maximum: 32});
...
function AddRegionesC(d){
	...
	'</fieldset>'+
	'</form>',size);
	$.when(promesas.all({pky:'lj.m,-/5tD',P:d.id})).done(function(x){
		x=jQuery.parseJSON(x);
		RegionesCR=[];
		selectDivisiones('.edUS fieldset label');
		if(x.errorMessage!='') creanotificacion('Error:',x.errorMessage,'','','error');
...
function selectDivisiones(c){
	...
	P.done(function(x){
		x=jQuery.parseJSON(x);
		if(x.errorMessage!='' && x.errorMessage!==null){
			creanotificacion('Error:',x.errorMessage,'','','error');
...
$(document).on("click","#tablaFielders tbody tr td a",function(event){event.preventDefault();
	...
	if(data.x=='Mensaje')
		...
		$.when(promesas.all({pky:'.tr/(ydF',P:data.id})).done(function(x){
			x=jQuery.parseJSON(x);
			if(x.errorMessage!=''){
				$("#loading").hide();
				creanotificacion('Error',x.errorMessage,'','','error');
			...
			if(size>0.75){
				...
				$.when($.ajax({url:Ñ,type:'POST',data:D,processData:false,contentType:false,})).done(function(x){
					...
					if(x.errorMessage!=''){
						console.log(x);
						b.removeClass('guardando').addClass('bad');
						creanotificacion('Mensaje',x.errorMessage,'','','error');
					}
					else{
						b.removeClass('guardando').addClass('ok');
	...
	if(c=='Y'){
		...
		if(h=='eliminoCamp'){
			...
			$.when(
				...
				x=jQuery.parseJSON(x);
				if(x.errorMessage!='') creanotificacion('No se eliminó la campaña',x.errorMessage,'','','error');
				muestraCampanas();
				repChecks=0;
				$('#loading').hide();
				$('#quest').dialog("close");
